...
const Login = () => {
  ...
  const { handlelogin, logInLoading,handleGoogleLogin } = useAuth()
        ...
        <div className="relative rounded-2xl border border-white/[0.08] bg-[#12151C]/90 backdrop-blur-xl shadow-[0_0_0_1px_rgba(255,255,255,0.02),0_20px_60px_-15px_rgba(0,0,0,0.7)] transition-all duration-300 hover:border-teal-400/20 hover:shadow-[0_0_0_1px_rgba(45,212,191,0.08),0_25px_70px_-15px_rgba(0,0,0,0.8),0_0_40px_-10px_rgba(45,212,191,0.15)] hover:-translate-y-0.5">
          ...
          <div className="px-5 pt-7 pb-6 sm:px-8 sm:pt-9 sm:pb-8">
            <h1 className="text-xl sm:text-[22px] font-semibold text-slate-50 tracking-tight">
              Welcome to <span className="text-teal-400 font-bold">PrepPilot-AI</span>
            </h1>
            ...
            <form onSubmit={handleSubmit} className="mt-6 sm:mt-7 space-y-4" noValidate>
              ...
              <div>
                ...
                <div className="group relative flex items-center rounded-lg border border-white/[0.08] bg-white/[0.03] transition-colors focus-within:border-teal-400/50 focus-within:bg-white/[0.04]">
                  <span className="pl-3 text-slate-500 group-focus-within:text-teal-400 transition-colors">
                    <LockIcon />
                  </span>
                  <input
                    ...
                    onChange={handleChange}
                    disabled={logInLoading}
                    className="w-full bg-transparent px-3 py-3 sm:py-2.5 text-sm text-slate-100 placeholder:text-slate-600 outline-none disabled:opacity-60"
                  />
                  <button
                    type="button"
              ...
              {error && (
                <p className="rounded-md border border-red-400/20 bg-red-400/[0.06] px-3 py-2 text-xs text-red-300">
                  {error}
                </p>
              )}
              ...
              <button
                type="submit"
                disabled={logInLoading}
                className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg bg-teal-400 px-4 py-3 sm:py-2.5 text-sm font-semibold text-[#0B0D12] transition-all hover:bg-teal-300 active:scale-[0.99] disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {logInLoading ? (
                  <>
